$(function () {
    ...
    $(window).on("scroll", function () {
        if ($(window).scrollTop() > 100) {
            ...
            $("#center>.rightbottom .Ticketing .VisitTime").animate({ width: 462, }, 2000);
            $("#center>.rightbottom .Ticketing  .VisitTime li:eq(0)").animate({ opacity: 1 }, 5000);
            $("#center>.rightbottom .Ticketing  .VisitTime li:eq(1)").animate({ opacity: 1 }, 6000);
            $("#center>.rightbottom .Ticketing  .VisitTime li:eq(2)").animate({ opacity: 1 }, 7000);
            $("#center>.rightbottom .Ticketing  .VisitTime li:eq(3)").animate({ opacity: 1 }, 8000);
        }
    })
    //获取界面滚出去距离的兼容性封装
    function getScroll() {
    ...
    function rotate1() {
        timeID_you1 = setInterval(function () {
            $("#center>.righttop>ul>li:eq(0) img").css({ 'transform': 'rotate(' + rotateDeg_you1 + 'deg)' });
            rotateDeg_you1 += 1;
        }, 100)
    }
    function rotate2() {
        timeID_you2 = setInterval(function () {
            $("#center>.righttop>ul>li:eq(1) img").css({ 'transform': 'rotate(' + rotateDeg_you2 + 'deg)' });
            rotateDeg_you2 += 1;
        }, 100)
    }
    function rotate3() {
        timeID_you3 = setInterval(function () {
            $("#center>.righttop>ul>li:eq(2) img").css({ 'transform': 'rotate(' + rotateDeg_you3 + 'deg)' });
            rotateDeg_you3 += 1;
        }, 100)
    }
    rotate1()
    ...
    function datumove(img) {
        ...
        timeID_datu_you = setInterval(function () {
            $(img[index_you]).siblings("li").css({ 'transform': 'scale(1)', opacity: 1 })
            $(img[index_you]).addClass('current').siblings("li").removeClass("current");
            $($("#datu>.dian_you>ul>li")[index_you]).addClass("current").siblings("li").removeClass("current");
