...
//ie6
	if ( $.browser.msie){
		 if(parseInt( $.browser.version )<9){
			 $("body").prepend("<div class='ie6' style='display:none;height:"+$(window).height()+"px'><span>关闭</span><a href='http://browsehappy.com/' target='_blank' style='text-decoration:underline'></a></div>");
  ...
  $('#nav > li').hover(
	  function(){
		  ...
		  NavWaitSlide = setTimeout(function() { 
			  if(!$(targ).is(':visible'))
			  {
				  	$(targ).css({left:0-($(targ).width()/2)+(current_li.outerWidth()/2),top:0+$(current_li).outerHeight()})
					$(targ).fadeIn('fast');
			  }
		  },100)
	  },
	...
	$(".bell").click(function(){
		...
		$(".masklayer").one("click",function(){$(".pop").hide();$(".masklayer").hide()})
	...
	$('body').bind("click", function(e){
		...
		$("#rightscrollnav .share").click(function(){bShare.more();})
	//分享返回顶部
	//mobile nav
